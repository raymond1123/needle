...
ApplyGemm(const Dtype* A, const Dtype* B, Dtype* C, int M, int N, int P) {
...
public:
    ...
    virtual cached_data_type compute(std::vector<cached_data_type> inputs) override {
        ...
        int N = inputs[0]->shape()[max_shape_len-1];
        int P = inputs[1]->shape()[max_shape_len-1];
        ...
        std::vector<int32_t> out_shape = {first_shape[0], M, P};
        ...
        final_shape.push_back(P);

        cached_data_type gemm_C = __create_cached_data(out_shape, inputs[0]->device());
        _n = gemm_C->size();
        ...
        for(int i=0; i < first_shape[0]; ++i) {
            ...
            ApplyGemm<Dtype><<<grid, block>>>(A, B, C, M, N, P);
        }
